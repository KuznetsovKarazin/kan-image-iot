...
def main():
    ...
    cmd = f"python scripts\\create_coco_train_minival_split.py --train_annotations_file=\"{train_ann}\" --val_annotations_file=\"{val_ann}\" --output_dir=\"{out_dir}\""
    run_command(cmd)
    ...
    cmd = f"python scripts\\create_visualwakewords_annotations.py --train_annotations_file=\"{maxitrain_ann}\" --val_annotations_file=\"{minival_ann}\" --output_dir=\"{vww_ann_dir}\" --threshold=0.005 --foreground_class=\"person\""
    run_command(cmd)
    
    # Step 5: Create subset for easier experimentation
    print("\nStep 5: Creating subset for experimentation")
    subset_dir = os.path.join(args.output_dir, 'processed', 'vww_subset')
